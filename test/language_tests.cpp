...
bool genVerilator = runCmd(genCmd);
  ...
  TEST_CASE("16 bit Newton Raphson") {
    ...
    SECTION("56 / 7 == 8") {
      ...
      sim.setInput("D", BitVector(16, 7));
      sim.evaluate();

      REQUIRE(sim.getOutput("Q") == BitVector(16, 56 / 7));
    }
    ...
    SECTION("239 / 17 == 14") {
      Simulator sim(*f);
      sim.setInput("N", BitVector(16, 239));
      sim.setInput("D", BitVector(16, 17));
      sim.evaluate();

      REQUIRE(sim.getOutput("Q") == BitVector(16, 239 / 17));
    }
